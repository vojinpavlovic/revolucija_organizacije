...
Rev.org.func.getOrg = function(name)
    if Config.Org.list[name] == nil then return end
...
Rev.Save = function()
    ...
    for k, v in pairs(Rev.org.jobs) do
        MySQL.Async.execute('UPDATE organisations SET money = @money, announces = @announces, poll = @poll WHERE name = @name', {
            ['@name'] = v.name,
            ['@money'] = v.money,
            ['@announces'] = json.encode(v.announces),
            ['@poll'] = json.encode(v.poll)
